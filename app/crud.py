...
class EdgeOperations(DBOperator):
    async def add(self, edges: List[scemas.EdgeIn]):
        proofs, edges = zip(*list(map(lambda edge: (
            models.Proof(date=edge.date_proof, proof=edge.proof, start_id=edge.start_id, end_id=edge.end_id),
            models.Edge(edge_type=edge.edge_type, start_id=edge.start_id, end_id=edge.end_id)), edges)))
